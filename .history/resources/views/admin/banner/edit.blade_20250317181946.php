                ...
                <div id="bannerFormSection" class="accordion accordion-flush filter-accordion">
                    <div class="card-body border-bottom">
                        ...
                        <form action="{{ route('banner.update', $banner->id) }}" method="POST"
                            ...
                            <!-- Hình ảnh -->
                            <div class="mt-3">
                                <label for="images">Ảnh Banner:</label>
                                <input type="file" id="images" name="images[]" class="form-control" multiple accept="image/*" onchange="previewMultipleImages(event)">
                                
                                <div id="imagePreviewContainer" style="display: flex; gap: 10px; margin-top: 10px;">
                                    @if (!empty($banner->image) && is_string($banner->image) && json_decode($banner->image, true))
                                        @foreach (json_decode($banner->image, true) as $image)
                                            <img src="{{ asset('storage/' . $image) }}" class="img-thumbnail" style="width: 100px; height: auto;">
                                        @endforeach
                                    @else
                                        <p>Chưa có ảnh</p>
                                    @endif
                            ...
                            <div class="filter-choices-input mt-3">
                                ...
                                <select name="active" class="form-control">
                                    <option value="1" {{ old('active', $banner->active) == 1 ? 'selected' : '' }}>Hoạt
                                        động</option>
                                    <option value="0" {{ old('active', $banner->active) == 0 ? 'selected' : '' }}>Không
                                        hoạt động</option>
                                </select>
                            </div>
