                    ...
                    <div class="card">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $item->title }}</td>
                                    <td><img src="{{ asset('storage/' . $item->image) }}" alt="Banner Image" width="100"></td>
                                    <td>{{ $item->content }}</td>
                                    <td>
                                        @if($item->link)
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2" role="group" aria-label="Basic example">
                                            <a href="{{ route('banner.edit', $item->id) }}" class="btn btn-primary">Sửa</a>
                                            <form action="{{ route('banner.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa banner này không?');" class="d-inline-block">
                                                @csrf
                                                @method('DELETE')
...
@push('scripts')
            ...
            columns: [{
                ...
                },
                {
                    ...
                    formatter: function(param) {
                        if (param.length > 0) {
                            return gridjs.html(
                                param.map(hinhAnh => '<img src="' + "{{ Storage::url('') }}" + hinhAnh
                                    .hinh_anh +
                                    '" width="50px" height="50px" style="margin-right: 5px;" alt="Đây là ảnh">'
                                    ).join('')
                            );
                        } else {
    ...
    <style>
        ...
        .status-an {
            ...
            /* Màu đỏ cho trạng thái Ẩn */
            color: #fff;
        }

        .status-hien {
            background-color: green;
            /* Màu xanh cho trạng thái Hiện */
            color: #fff;
        }
        ...
        .status-an:hover {
            background-color: red;
            /* Giữ nguyên màu đỏ cho nút trạng thái Ẩn */
            color: #fff;
        }

        .status-hien:hover {
            background-color: green;
            /* Giữ nguyên màu xanh cho nút trạng thái Hiện */
            color: #fff;
        }
        ...
        .btn-group-sm .dropdown-menu {
            min-width: 100px;
            /* Tăng kích thước chiều rộng của menu */
        }
    </style>
@endpush
