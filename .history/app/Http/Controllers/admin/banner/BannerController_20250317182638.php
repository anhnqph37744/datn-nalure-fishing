...
class BannerController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Nếu có ảnh mới, xóa ảnh cũ và lưu mới
        if ($request->hasFile('images')) {
            // Kiểm tra nếu có ảnh cũ thì xóa
            if (!empty($banner->image)) {
                $oldImages = json_decode($banner->image, true) ?? []; // Đảm bảo luôn là mảng
        
                if (is_array($oldImages)) { // Tránh lỗi nếu không phải mảng
            ...
            $banner->image = json_encode($imagePaths, JSON_UNESCAPED_SLASHES);
        } elseif (empty($banner->image)) { // Kiểm tra nếu không có ảnh, gán mảng rỗng
